	...
	<div itemscope itemtype="https://schema.org/Recipe">
		@if ($row->filename)
			<div id="recipe-image-container">
				<img alt="" height="562" id="recipe-image" itemprop="image" src="/uploads/{{ $row->filename }}" width="1024" />
		...
		<div id="recipe">
			<header id="recipe-header">
				<h1>
					<span itemprop="name">{{ $row->title }}</span> Recipe
					@if ($row->is_private)
					...
					<footer id="recipe-footer">
						...
						@if ($row->serving_size)
							...
							<dl itemprop="nutrition" itemscope itemtype="https://schema.org/NutritionInformation">
								<meta itemprop="servingSize" content="{{ $row->serving_size }}" />
								@if ($row->calories)
									<div class="line"><dt>Calories</dt><dd itemprop="calories">{{ $row->calories }}</dd></div>
								@endif
								<div class="double">
									@if ($row->fat)
										<div class="line"><dt>Fat</dt><dd itemprop="fatContent">{{ $row->fat }} <span aria-label="grams">g</span></dd></div>
									@endif
									@if ($row->saturated_fat)
										<div class="line indent"><dt>Saturated</dt><dd itemprop="saturatedFatContent">{{ $row->saturated_fat }} <span aria-label="grams">g</span></dd></div>
									@endif
									@if ($row->trans_fat)
										<div class="line indent"><dt>+ Trans</dt><dd itemprop="transFatContent">{{ $row->trans_fat }} <span aria-label="grams">g</span></dd></div>
									@endif
								...
								@if ($row->cholesterol)
									<div class="line"><dt>Cholesterol</dt><dd itemprop="cholesterolContent">{{ $row->cholesterol }} <span aria-label="milligrams">mg</span></dd></div>
								@endif
								@if ($row->sodium)
									<div class="line"><dt>Sodium</dt><dd itemprop="sodiumContent">{{ $row->sodium }} <span aria-label="milligrams">mg</span></dd></div>
								@endif
								@if ($row->potassium)
								...
								<div class="double">
									@if ($row->carbohydrate)
										<div class="line"><dt>Carbohydrate</dt><dd itemprop="carbohydrateContent">{{ $row->carbohydrate }} <span aria-label="grams">g</span></dd></div>
									@endif
									@if ($row->fibre)
										<div class="line indent"><dt>Fibre</dt><dd itemprop="fiberContent">{{ $row->fibre }} <span aria-label="grams">g</span></dd></div>
									@endif
									@if ($row->sugars)
										<div class="line indent"><dt>Sugars</dt><dd itemprop="sugarContent">{{ $row->sugars }} <span aria-label="grams">g</span></dd></div>
									@endif
								</div>
								@if ($row->protein)
									<div class="line"><dt>Protein</dt><dd itemprop="proteinContent">{{ $row->protein }} <span aria-label="grams">g</span></dd></div>
								@endif
								<div class="double">
									@if ($row->vitamin_a)
